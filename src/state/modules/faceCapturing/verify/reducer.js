import {
  START_RECORDER_COUNTDOWN, RECORDING_COUNTDOWN, VERIFYING_COUNTDOWN, SET_PASSPHRASE, STREAM
} from './actions';
import { RECORDED } from '../recorder/actions';
import {
  FACE_IMAGE, FACE_RESULTS, START_LOAD_FACEAPI, START_DETECTING_FACE
} from '../face/actions';
...
export default function reducer(state = { currentMs: null, ticks: Array(numberOfTicks).fill(0) }, action) {
  switch (action.type) {
    case VERIFYING_COUNTDOWN:
      return {
        ...state,
        verifyCountdown: action.payload
      };
    case STREAM:
      return {
        ...state,
    ...
    case RECORDING_COUNTDOWN:
      return {
        ...
        recordingCoundown: action.payload,
      };
    case SET_PASSPHRASE:
      return {
        ...state,
